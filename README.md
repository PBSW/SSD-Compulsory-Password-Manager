...
Although the solution is completely dockerized, it still uilizes a SQLite database, which is suboptimal for security as it provides no access-control mechanisms. While perfectly fine for development environments, an SQL Server instance should be spun up with non-default credentials to further reduce the risk of insider threats. Speaking of docker, the docker image for the frontend is suboptimal, as it is not intended for production environments. It evens provides a warning: ` frontend  | Warning: This is a simple server for use in testing or debugging Angular applicationsfrontend  | locally. It hasn't been reviewed for security issues.`. This was a last resort due to issues with nginx.
